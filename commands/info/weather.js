...
module.exports = class WeatherCommand extends Command {
	...
	async run(message, args) {
		...
		const { body: location } = await get(`https://maps.googleapis.com/maps/api/geocode/json?address=${args}&key=${keys.google}`);
		...
		const { body: weather } = await get(`https://api.darksky.net/forecast/${keys.darksky}/${latitude},${longitude}`);
		...
		icon.src = path.join(__dirname, '..', '..', 'assets', 'images', `${getIcon(weather.currently.icon)}.png`);
		...
		for (let i = 0; i < 4; i++) {
			...
			newImage.src = path.join(__dirname, '..', '..', 'assets', 'images', `${getIcon(weather.daily.data[i].icon)}.png`);
